...
class ImageDataGenerator3D(object):
    ...
    def apply_transform(self, x, transform_parameters):
        ...
        img_row_axis = self.row_axis - 1
        img_col_axis = self.col_axis - 1
        img_height_axis = self.height_axis-1
        img_channel_axis = self.channel_axis - 1

        for i in range(x.shape[img_height_axis]):
            if img_height_axis == 0:
                temp_x = x[i,:,:,:]
            if img_height_axis == 1:
                temp_x = x[:,i,:,:]
            temp_x = apply_affine_transform(temp_x, transform_parameters.get('theta', 0),
                                        ...
                                        channel_axis=img_channel_axis-1,
                                        fill_mode=self.fill_mode,
                                        cval=self.cval)

            if transform_parameters.get('channel_shift_intensity') is not None:
                temp_x = apply_channel_shift(temp_x,
                                        transform_parameters['channel_shift_intensity'],
                                        img_channel_axis)

            if transform_parameters.get('flip_horizontal', False):
                temp_x = flip_axis(temp_x, img_col_axis)

            if transform_parameters.get('flip_vertical', False):
                temp_x = flip_axis(temp_x, img_row_axis)

            if transform_parameters.get('brightness') is not None:
                temp_x = apply_brightness_shift(temp_x, transform_parameters['brightness'])
            if img_height_axis == 0:
                x[i,:,:,:] = temp_x
            if img_height_axis == 1:
                x[:,i,:,:] = temp_x
    ...
    def fit(self, x,
       ...
       """
        ...
        if x.shape[self.channel_axis] not in {1, 4, 5}:
            warnings.warn(
        ...
        if self.featurewise_center:
            self.mean = np.mean(x, axis=(0, self.row_axis, self.col_axis, self.height_axis))
        ...
        if self.featurewise_std_normalization:
            self.std = np.std(x, axis=(0, self.row_axis, self.col_axis, self.height_axis))
            broadcast_shape = [1, 1, 1, 1]
            broadcast_shape[self.channel_axis - 1] = x.shape[self.channel_axis]
...
class NumpyArrayIterator3D(Iterator):
    ...
    def __init__(self, x, y, image_data_generator,
        ...
        if self.x.shape[channels_axis] not in {1, 4, 5}:
            warnings.warn('NumpyArrayIterator is set to use the '
                          'data format convention "' + data_format + '" '
                          '(channels on axis ' + str(channels_axis) +
                          '), i.e. expected either 1, 4, or 5 '
                          'channels on axis ' + str(channels_axis) + '. '
                          'However, it was passed an array with shape ' +
                          str(self.x.shape) + ' (' +
        ...
        self.save_prefix = save_prefix
        self.save_format = save_format
        super(NumpyArrayIterator3D, self).__init__(x.shape[0],
                                                 batch_size,
                                                 shuffle,
                                                 seed)
